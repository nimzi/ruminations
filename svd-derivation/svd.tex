...
axes). Every example does this. So the answer to the question looks like
\emph{yes} --- but examples are not a proof, and it is not yet clear why it should
be true or whether the perpendicular preimage pair is unique. The next sections
settle both, first in the plane, then in general.
...
frame to the axes: $A\vv i = \sigma_i \uu i$, just as in the plane.
...
gives $AV = U\Sigma$, i.e.\ $A = U\Sigma V\T$. The construction of
\S\S\ref{sec:existence}--\ref{sec:algebraic} has, without announcing it, proved
exactly this --- so it is worth stating plainly, as the theorem the whole first
half was after.
...
\noindent
The boxed relation is the same statement seen through its columns:
