...
\title{\textbf{Why Zero-Centered Hidden Activations Help Gradient Descent}\\
\large From sigmoid vs. tanh to gradient geometry, zig-zagging, and conditioning}
\author{}
\date{}
...
Consider a downstream neuron with inputs \(h_1,\ldots,h_m\):
\begin{equation}
  z=\sum_{j=1}^{m}w_j h_j+b,
  \qquad
  y=\phi(z).
\end{equation}
For one training example, define the backpropagated scalar
\begin{equation}
  \delta \equiv \frac{\partial L}{\partial z}.
\end{equation}
Then
\begin{equation}
  \boxed{\frac{\partial L}{\partial w_j}=\delta h_j}
  \label{eq:basicgrad}
\end{equation}
and
\begin{equation}
...
Equation \cref{eq:basicgrad} is the heart of the Stanford CS231n intuition.  If the previous hidden layer uses an activation whose output is always positive, such as a logistic sigmoid, then
\[
  h_j>0 \quad \text{for all }j.
\]
Therefore, for that single example,
\begin{equation}
  ...
  \label{eq:samesign}
\end{equation}
All components of the gradient vector have the same sign.  The gradient lies in an ``all positive'' orthant or an ``all negative'' orthant.

If the hidden activations are centered and may take both signs, the components \(\delta h_j\) need not share a sign.  A single example can immediately produce directions such as
\[
  (+,-,+,-,\ldots),
\]
which are unavailable when every \(h_j\) is positive.

\subsection{The upstream gradient may be positive or negative even when activations are positive}

The fact that a sigmoid output \(y\) is positive does \emph{not} imply that \(\partial L/\partial y\) is positive.  For squared error,
\[
  L=\frac12(y-t)^2,
  \qquad
  \frac{\partial L}{\partial y}=y-t.
\]
If \(y=0.3\) and \(t=1\), then \(\partial L/\partial y=-0.7\).  If \(y=0.7\) and \(t=0\), then \(\partial L/\partial y=+0.7\).  Activations describe the current coordinates; gradients describe which infinitesimal direction increases the loss.

For a hidden neuron, the incoming upstream gradient is itself a weighted combination of downstream errors, so it can again have either sign.  What positivity of \(h_j\) does is not fix the sign of the scalar \(\delta\); it makes \emph{all weight-gradient components inherit the same sign from that scalar} for a single example.

\section{The zig-zag phenomenon}

Stanford CS231n makes the sign restriction geometrically intuitive.  Consider only two weights, \(w_1,w_2\).  With positive hidden inputs \(h_1,h_2>0\), the per-example gradient satisfies
\[
  \nabla_w L=\delta(h_1,h_2).
\]
Hence the gradient, and therefore the negative-gradient update, must lie in one of two opposite quadrants: both components have the same sign.
...
\begin{tikzpicture}[scale=1.02,>=Latex]
  ...
  \fill (O) circle (2.2pt) node[above left=2pt] {current $w$};
  ...
  \node[accent,align=center,fill=white,inner sep=2pt] at (-2.05,-2.15)
    {dashed arrow = desired net change\\$\Delta w=(+,-)$};
...
\end{tikzpicture}
\caption{Corrected schematic of the CS231n ``zig-zag'' intuition.  The blue arrows are gradient-descent updates, $\Delta w=-\eta\nabla_w L$.  Every individual blue arrow has same-sign components---it points northeast $(+,+)$ or southwest $(-,-)$ relative to its own tail.  Nevertheless, alternating such steps can accumulate to the mixed-sign net displacement indicated by the dashed red arrow.  The picture is illustrative, not a literal trajectory for every loss surface.}
\label{fig:zigzag}
\end{figure}
...
\begin{itemize}
  ...
  \[
    \frac{\partial L}{\partial w_j}=\sum_i \delta_i h_{ij},
  \]
...
\boxed{
\begin{array}{c}
\text{same function class}\\
+\ \text{same derivative shape}\\
+\ \text{following bias}\\[3pt]
\Longrightarrow\\[-2pt]
\text{centering mainly changes optimization geometry, not expressivity.}
\end{array}}
\end{equation}
